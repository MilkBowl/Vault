...
import com.iConomy8.util.Constants;
import com.iConomy8.iConomy;
import com.iConomy8.system.Accounts;
import com.iConomy8.system.Holdings;

public class Economy_iConomy8 extends AbstractEconomy {
    ...
    public Economy_iConomy8(Plugin plugin) {
        this.plugin = plugin;
        Bukkit.getServer().getPluginManager().registerEvents(new EconomyServerListener(this), plugin);
        log.warning("It is recommended to use SQL with iConomy8 !");
        // Load Plugin in case it was loaded before
        if (economy == null) {
            Plugin ec = plugin.getServer().getPluginManager().getPlugin("iConomy");
            if (ec != null && ec.isEnabled() && ec.getClass().getName().equals("com.iConomy8.iConomy")) {
    ...
    public class EconomyServerListener implements Listener {
        Economy_iConomy8 economy = null;

        public EconomyServerListener(Economy_iConomy8 economy) {
    ...
    public double getBalance(String playerName) {
        if (accounts.exists(playerName)) {
            return accounts.get(playerName).getHoldings().balance();
        } else {
            return 0;
        }
    }
    ...
    public EconomyResponse withdrawPlayer(String playerName, double amount) {
        ...
        if (holdings.hasEnough(amount)) {
            ...
            return new EconomyResponse(0, holdings.balance(), ResponseType.FAILURE, "Insufficient funds");
        }
    }
    ...
    public EconomyResponse createBank(String name, String player) {
        if (accounts.exists(name)) {
            return new EconomyResponse(0, accounts.get(name).getHoldings().balance(), ResponseType.FAILURE, "That account already exists.");
        }
    ...
    public EconomyResponse bankHas(String name, double amount) {
        if (has(name, amount)) {
            ...
            return new EconomyResponse(0, accounts.get(name).getHoldings().balance(), ResponseType.FAILURE, "The account does not have enough!");
        }
    }
    ...
    public EconomyResponse bankBalance(String name) {
        if (!accounts.exists(name)) {
            ...
            return new EconomyResponse(0, accounts.get(name).getHoldings().balance(), ResponseType.SUCCESS, null);
        }
    }
